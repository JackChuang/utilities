...
print("dynamically importing data from: %s%s" % (CENSUS[:-3], CENSUS[-3:]))
census = __import__('%s' % (CENSUS[:-3]))
pgfault_census = census

pgfault_census_sorted = OrderedDict((k, v) for k, v in sorted(pgfault_census.AllData.items(), key=lambda x: x[1], reverse=True))
...
sys.stdout.flush()
i=0
for k, v in pgfault_census_sorted.items():
    if v > threshold:
        print k, v
        i += 1
        if i >= 20:
