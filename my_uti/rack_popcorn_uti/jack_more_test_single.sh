...
RESULT_FOLDER=result12_mimicw_ibverb
TARGET_NODE=echo6
TEST=12
TEST2=
#TEST=13 // free
...
do
	...
	do
		...
		do
			if [ $jack -eq 1 ]; then
				ssh $TARGET_NODE rm $RESULT_FOLDER$iter/node6_"$TEST"_"$t"_"$j"
			else
				ssh $TARGET_NODE rm $RESULT_FOLDER$iter/node6_"$TEST2"_"$t"_"$j"
			fi
			ssh $TARGET_NODE sudo dmesg -c \> /dev/null
			
			for i in {6..6..1}	# run on echo6
			do
	...
	do
		echo ""
		ssh echo6 echo 0 \> /proc/kmsg_test
	done
done
done
